...
workers = users[:500]
clients = users[500:]
privilegeAssignments = [PrivilegeAssignment(user.Id_use, privilege.Id_privilege)
                        for user in workers
                        for privilege in random.choices(privileges[2:], k=random.randrange(1, 3))] + \
                       [PrivilegeAssignment(user.Id_use, random.choice(privileges[:2]).Id_privilege)
                        for user in clients]
raitings = [Rating(movie.Id_movie, user.Id_use)
...
reservations = [Reservation(seans.Id_seans, random.choice(reservationStates).Id_reservationState, user.Id_use)
                for user in clients
